...
void CGraphicsBackend_SDL_OpenGL::ShaderSet(int ID) { if(ID < NUM_SHADERS) m_ShaderID = ID; }

void CGraphicsBackend_SDL_OpenGL::ShaderBegin()
{
	if(!m_ShaderEnable)
		return;
...
void CGraphicsBackend_SDL_OpenGL::ShaderEnd()
{
	if(!m_ShaderEnable)
		return;
...
void CGraphicsBackend_SDL_OpenGL::ShaderUniformSet(const char* name, float *var, int num)
{
	if(!m_ShaderEnable)
		return;
...
void CGraphicsBackend_SDL_OpenGL::ShaderLoad(const char *vs, const char *fs)
{
	if(!m_ShaderEnable)
		return;

	FILE *f;
	char *vertexSource = (char*)malloc(10240);
	char *fragmentSource = (char*)malloc(10240);
	f = fopen(vs, "r");
	if(f)
	{
		fread(vertexSource, 10240, 10240, f);
		fclose(f); f = 0;
	}
	f = fopen(fs, "r");
	if(f)
	{
		fread(fragmentSource, 10240, 10240, f);
		fclose(f); f = 0;
	}
	...
	glShaderSourceARB(vsHandle, 1, (const GLchar**)&vertexSource, NULL);
	free(vertexSource);
	glShaderSourceARB(fsHandle, 1, (const GLchar**)&fragmentSource, NULL);
	free(fragmentSource);
...
void CGraphicsBackend_SDL_OpenGL::FrameBufferToScreen()
{
	float var[2];
...
void CGraphicsBackend_SDL_OpenGL::UseFrameBuffer(bool active)
{
	if(active)
		glBindFramebufferEXT(GL_FRAMEBUFFER_EXT, m_Fbo);
	else
...
void CGraphicsBackend_SDL_OpenGL::FrameBufferInit()
{
...
void CGraphicsBackend_SDL_OpenGL::FrameBufferDepthBufferInit()
{
...
void CGraphicsBackend_SDL_OpenGL::FrameBufferTextureInit()
{
...
void CGraphicsBackend_SDL_OpenGL::ShaderInit()
{
