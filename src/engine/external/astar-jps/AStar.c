...
#define directionset unsigned char
...
static int getIndex (struct coord_t bounds, struct coord_t c)
{
...
static struct coord_t getCoord (struct coord_t bounds, int c)
{
	struct coord_t rv = { c % bounds.x, c / bounds.x };
	return rv;
}
...
static int contained (struct coord_t bounds, struct coord_t c)
{
	return c.x >= 0 && c.y >= 0 && c.x < bounds.x && c.y < bounds.y;
}

// is this coordinate within the map bounds, and also walkable?
static int isEnterable (astar_t *astar, struct coord_t coord)
{
...
static struct coord_t adjustInDirection (struct coord_t c, int dir)
{
 ...
 */
/*
static int hasForcedNeighbours (astar_t *astar, struct coord_t coord, int dir)
{
...
static void addToOpenSet (astar_t *astar,
			  ...
			  int nodeFrom)
{
	struct coord_t nodeCoord = getCoord (astar->bounds, node);
	struct coord_t nodeFromCoord = getCoord (astar->bounds, nodeFrom);
...
{
	struct coord_t coord = adjustInDirection (getCoord (astar->bounds, start), dir);
...
static int nextNodeInSolution (astar_t *astar,
			       ...
			       int node)
{
	struct coord_t c = getCoord (astar->bounds, node);
	struct coord_t cTarget = getCoord (astar->bounds, *target);
...
static direction directionOfMove (struct coord_t to, struct coord_t from)
{
	if (from.x == to.x) {
		if (from.y == to.y)
			return -1;
