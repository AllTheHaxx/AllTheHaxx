...
#define PUTLINE(LINE) \
			...
			{ \
				...
				UI()->DoLabelScaled(&Label, aBuf+1, 20.0f, -1, (int)MainView.w); \
				for(int i = 1; i < TextRender()->TextLineCount(0, 20.0f, LINE, (int)MainView.w); i++) MainView.HSplitTop(30.0f, &Label, &MainView); \
			} \
			else \
			{ \
				MainView.HSplitTop(20.0f, &Label, &MainView); \
				UI()->DoLabelScaled(&Label, LINE, 15.f, -1, (int)(MainView.w - 30.0f)); \
				for(int i = 1; i < TextRender()->TextLineCount(0, 15.0f, LINE, (int)(MainView.w - 30.0f)); i++) \
					MainView.HSplitTop(20.0f, &Label, &MainView); \
			}
