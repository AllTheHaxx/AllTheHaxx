...
void CgSkins::OnInit()
{
	...
	if(!m_aSkins.size())
	{
		Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "gameclient", "failed to load textures. folder='gameskins/'");
		CgSkin DummySkin;
...
void CgSkins::OnReset()
{
	g_pData->m_aImages[IMAGE_GAME].m_Id = Get(Find(g_Config.m_GameTexture))->m_Texture;
}
