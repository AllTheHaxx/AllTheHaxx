...
*/	)
{
	...
	// draw gun
	{
		if(((pPlayerInfo->m_Local && (GameClient()->m_pControls->m_InputData[g_Config.m_ClDummy].m_PlayerFlags&PLAYERFLAG_AIM)) || (Player.m_PlayerFlags&PLAYERFLAG_AIM)) && (g_Config.m_ClShowOtherHookColl || pPlayerInfo->m_Local))
		{
			float Alpha = 1.0f;
			if (OtherTeam)
	...
	// render anti-ping'd nameplates if needed
	if(g_Config.m_ClNameplates && m_pClient->AntiPingPlayers())
	{
		CNetObj_Character prev = *pPrevChar;
		CNetObj_Character now = *pPlayerChar;
		prev.m_X = Position.x; prev.m_Y = Position.y;
		now.m_X = Position.x; now.m_Y = Position.y;
		GameClient()->m_pNamePlates->RenderNameplate(&prev, &now, pPlayerInfo);
	}
}
